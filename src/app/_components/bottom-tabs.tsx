    ...
    <nav className="no-print fixed inset-x-0 bottom-0 z-50 border-t border-[var(--border)] bg-[var(--card)]/95 backdrop-blur-xl lg:hidden">
      <div className="mx-auto flex max-w-lg items-stretch justify-around">
        {TABS.map((tab) => {
          ...
          return (
  <nav className="no-print fixed inset-x-0 bottom-0 z-50 border-t border-[var(--border)] bg-[#fbfaf7]/95 shadow-[0_-8px_30px_rgba(23,23,20,0.06)] backdrop-blur-xl lg:hidden">
        ...
        return (
                  ...
                  >
                    <Icon
                      ...
                      className={active ? "text-[var(--accent)]" : "text-[#b9b7b0]"}
                    />
                    <span>{tab.label}</span>
                  </Link>
                );
